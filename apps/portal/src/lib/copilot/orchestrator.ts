...
export async function orchestrate(input: OrchestratorInput): Promise<OrchestratorResult> {
  ...
  let context = buildCopilotContext({
    memory,
  });
  ...
  const responseText = generateResponseText(intent, context, evidence, request.query);
  ...
  const relatedBenchmarks = context.benchmarkHistory
    .slice(0, 3)
    .map((b) => b.name);
...
export function generateDailyBrief(context: CopilotContext): DailyBrief {
