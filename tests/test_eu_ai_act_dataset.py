...
class TestQuestions:
    ...
    def test_no_near_duplicate_questions(self, queries: list[dict]) -> None:
        ...
        assert not pairs, "Near-duplicate questions found: " + "; ".join(
            f"{a} ↔ {b} ({r:.2f})" for a, b, r in pairs
        )
...
class TestReferences:
    ...
    def test_all_references_valid(self, queries: list[dict]) -> None:
        ...
        assert not invalid, "Invalid references: " + "; ".join(
            f"{qid}: {ref}" for qid, ref in invalid
        )

    def test_all_expected_articles_non_empty(self, queries: list[dict]) -> None:
        for q in queries:
            articles = q.get("expected_articles")
            assert articles, f"{q['id']}: expected_articles is empty/null"

    def test_expected_chunks_is_list_or_null(self, queries: list[dict]) -> None:
        for q in queries:
...
class TestConfidenceBand:
    ...
    def test_confidence_band_valid_values(self, queries: list[dict]) -> None:
        valid = {"high", "medium", "low"}
        for q in queries:
            band = q.get("confidence_band")
            assert band in valid, f"{q['id']}: invalid confidence_band {band!r}"
...
class TestMetadata:
    ...
    def test_difficulty_valid_values(self, queries: list[dict]) -> None:
        ...
        for q in queries:
            diff = q.get("difficulty")
            assert diff in valid, f"{q['id']}: invalid difficulty {diff!r}"
