...
class TestFeedbackCollector:
    def test_record_feedback(self, collector) -> None:
        collector.record_feedback(
            query_id="q001", query="What is AI?", query_type="SIMPLE",
            retrieval_type="HYBRID", confidence=0.95,
            calibrated_confidence=0.92, top_k=3, rerank=False,
    ...
    def test_record_feedback_invalid_rating(self, collector) -> None:
        with pytest.raises(ValueError):
            collector.record_feedback(
                ...
                retrieval_type="HYBRID", confidence=0.9,
                calibrated_confidence=0.9, top_k=3, rerank=False,
                decompose=False, answer_accepted=True, answer_rating=0,
            )

    def test_record_feedback_missing_query_id(self, collector) -> None:
        with pytest.raises(ValueError):
            collector.record_feedback(
                query_id="", query="test", query_type="SIMPLE",
                retrieval_type="HYBRID", confidence=0.9,
                calibrated_confidence=0.9, top_k=3, rerank=False,
                decompose=False, answer_accepted=True,
            )
    ...
    def test_export_dataset_with_stored(self, collector, temp_storage) -> None:
        ...
        collector.flush()
        collector.record_feedback(
            query_id="q002", query="test2", query_type="COMPLEX",
