...
class ComplexRetriever(BaseRetriever):
    ...
    def retrieve_top_k(
        ...
        if decompose:
            ...
            dense_retrieval = self.store.query(
                namespace, top_k, query_embed, return_embeddings=True
            )
            hypothesis_retrieval = self.store.query(
                namespace, top_k, hypothetical_answer_embed, return_embeddings=True
            )
            sub_query_retrieval = self.store.query(
                namespace, top_k, sub_query_embed, return_embeddings=True
            )

            all_retrieval = (
                dense_retrieval["documents"][0]
                + hypothesis_retrieval["documents"][0]
                + sub_query_retrieval["documents"][0]
            )
            ...
            all_embeddings = (
                dense_embeddings + hypothesis_embeddings + sub_query_embeddings
            )
        else:
            dense_retrieval = self.store.query(
                namespace, top_k, query_embed, return_embeddings=True
    ...
    def _mmr_calc(
        ...
        candidate_embeds: list[list[float]],
        top_k: int,
        lambda_val: float,
    ) -> list[str]:
        ...
        candidate_norms = candidate_embeds / (
            np.linalg.norm(candidate_embeds, axis=1, keepdims=True) + 1e-9
        )  # Adding this small value to prevent division by zero error
        ...
        for _ in range(min(top_k, len(candidate_docs))):
            if not selected_indices:
                ...
                similarity_to_selected = (
                    candidate_norms[remaining_indices] @ selected_embeds.T
                )
    ...
    def _llm_embeddings(self, prompt: str):
        if self.llm_provider == "gemini":
            try:
                hypothesis_answer = self.llm_client.models.generate_content(
