...
def compute_budget_accuracy(
    ...
    for entry in entries:
        ...
        if rec.recommended_top_k == entry.top_k and rec.recommended_rerank == entry.rerank and rec.recommended_decompose == entry.decompose:
            correct += 1
        total += 1
...
def compute_success_lift(
    ...
    for entry in entries:
        ...
        seen.add(dedup)
        rec = optimizer.recommend_budget(entry.query_type, entry.confidence)
        if rec.recommended_top_k == entry.top_k and rec.recommended_rerank == entry.rerank and rec.recommended_decompose == entry.decompose and entry.success:
            learned_successes += 1
        total += 1
...
def compute_latency_delta(
    ...
    learned_avg = sum(learned_latencies) / len(learned_latencies) if learned_latencies else 0.0
...
def compute_fallback_reduction(
    ...
    for entry in entries:
        ...
        seen.add(dedup)
        rec = optimizer.recommend_budget(entry.query_type, entry.confidence)
        if rec.recommended_top_k == entry.top_k and rec.recommended_rerank == entry.rerank and rec.recommended_decompose == entry.decompose and entry.fallback_triggered:
            learned_fallbacks += 1
        total += 1
...
def generate_optimization_report(
    ...
    for qt in sorted(table.mapping):
        ...
        lines.append("")
        lines.append("| Band | top_k | rerank | decompose | Expected Success | Expected Latency (ms) |")
        lines.append("| ---- | ----- | ------ | --------- | ---------------- | --------------------- |")
    ...
    if lift > 0:
        lines.append("**Verdict: PRODUCTION READY** — Learned budget outperforms static budget.")
    else:
        lines.append("**Verdict: NEEDS MORE DATA** — Score not improving, collect more telemetry.")
