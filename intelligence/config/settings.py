...
class Settings(BaseSettings):
    ...
    deployment: bool = Field(default=False, description="Enable production deployment mode")
    ...
    health_check_enabled: bool = Field(default=True, description="Enable gRPC health check")
    ...
    api_cors_origins: list[str] = Field(default=["*"], description="Allowed CORS origins")
    api_rate_limit: int = Field(default=100, description="Default API rate limit per minute")
    api_rate_limit_burst: int = Field(default=200, description="Default API rate limit burst")
    api_secret: Optional[str] = Field(default=None, description="API shared secret for authentication")
    ...
    # --- Embedding ----------------------------------------------------
    embedding_model: str = Field(default="local", description="Embedding provider: local, openai, gemini")

    # --- LLM ----------------------------------------------------------
    llm_provider: Optional[str] = Field(default=None, description="LLM provider: gemini, openai, ollama")
    gemini_api_key: Optional[str] = Field(default=None, description="Gemini API key")
    gemini_model_name: Optional[str] = Field(default=None, description="Gemini model name")
    openai_api_key: Optional[str] = Field(default=None, description="OpenAI API key")
    openai_model_name: Optional[str] = Field(default=None, description="OpenAI model name")
    ollama_model_name: Optional[str] = Field(default=None, description="Ollama model name")
    ...
    large_groq_model: Optional[str] = Field(default=None, description="Groq large model name")
    small_groq_model: Optional[str] = Field(default=None, description="Groq small model name")
    provider_timeout_seconds: float = Field(default=30.0, description="LLM provider timeout")

    # --- Retrieval ----------------------------------------------------
    chunk_size: int = Field(default=1024, description="Document chunk size")
    overlap: int = Field(default=150, description="Chunk overlap")
    mmr_retrieval_lambda: float = Field(default=0.5, description="MMR diversity lambda (0=max div, 1=max rel)")
    ...
    circuit_breaker_recovery_timeout: float = Field(default=30.0, description="Seconds before half-open retry")

    # --- Dashboard ----------------------------------------------------
    dashboard_port: int = Field(default=8501, description="Streamlit dashboard port")

    # --- Artifacts ----------------------------------------------------
    artifacts_dir: str = Field(default="./artifacts", description="Artifact storage directory")
    model_registry_dir: str = Field(default="./models", description="Model registry directory")
    experiment_registry_dir: str = Field(default="./experiments", description="Experiment registry directory")
    report_output_dir: str = Field(default="./reports", description="Report output directory")

    # --- Docker -------------------------------------------------------
    docker_registry: str = Field(default="", description="Docker registry URL for images")
    ...
    coverage_threshold: float = Field(default=70.0, description="Minimum coverage percentage")
