...
def compute_config_win_rate(
    ...
    return {
        cfg: compute_acceptance_rate(group)
        for cfg, group in sorted(groups.items())
    }
...
def compute_strategy_win_rate(
    ...
    return {
        strat: compute_acceptance_rate(group)
        for strat, group in sorted(groups.items())
    }
...
def generate_feedback_report(
    ...
    success_rate = sum(1 for r in records if r.retrieval_success) / len(records) if records else 0.0
    fallback_rate = sum(1 for r in records if r.fallback_triggered) / len(records) if records else 0.0
