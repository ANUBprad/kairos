...
class BudgetRetrainer:
    ...
    def generate_training_report(
        ...
        for key in ["avg_score", "success_rate", "avg_latency", "fallback_rate"]:
            ...
            learned_val = eval_results.get(learned_key, 0.0)
            if key == "avg_latency":
                delta = learned_val - s
                lines.append(
                    f"| {key.replace('_', ' ').title():<20} | {s:.1f} | {learned_val:.1f} | {delta:+.1f} |"
                )
            else:
                delta = learned_val - s
                lines.append(
                    f"| {key.replace('_', ' ').title():<20} | {s:.2%} | {learned_val:.2%} | {delta:+.2%} |"
                )
        ...
        for cfg, accepts in sorted(configs.items()):
            ...
            lines.append(
                f"| {cfg:<25} | {len(accepts):<5} | {sum(accepts):<8} | {rate:.2%} |"
            )
        ...
        for r in records:
            entries.append(
                BudgetDatasetEntry(
                    ...
                    success=r.get("accepted", True)
                    and not r.get("fallback_triggered", False),
                )
            )
        return entries
